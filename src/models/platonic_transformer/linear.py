...
class PlatonicLinear(nn.Module):
    """
    Group-aware linear projection for dense token sets.

    The weights are parameterised so that each Platonic group element owns a slice
    of the kernel. Calling :meth:`forward` applies the appropriate slice to every
    token while respecting the discrete symmetry.
    """

    def __init__(
        self, in_features: int, out_features: int, solid: str, bias: bool = True
    ):
        super().__init__()
        ...
        if in_features % self.G != 0:
            raise ValueError(
                f"in_features ({in_features}) must be divisible by the group order {self.G} for solid '{solid}'"
            )
        if out_features % self.G != 0:
            raise ValueError(
                f"out_features ({out_features}) must be divisible by the group order {self.G} for solid '{solid}'"
            )
        ...
        self.kernel = nn.Parameter(
            torch.empty(self.G, self.out_channels, self.in_channels)
        )
    ...
    def get_weight(self) -> Tensor:
        """
        Build the flattened weight matrix with shape [G*out_channels, G*in_channels].
        Every row/column pair corresponds to a specific group element so that the
        full operator respects the discrete symmetry.
        """
    ...
    def forward(self, x: Tensor) -> Tensor:
        """Apply the linear map to a batch of tokens of shape [..., G*in_channels]."""
...
def run_equivariance_test(solid_name: str):
    ...
    print(
        f"  (G={G}, I={I}, O={O}) -> in_features={in_feats}, out_features={out_feats}"
    )
    layer = PlatonicLinear(
        in_features=in_feats, out_features=out_feats, solid=solid_name
    )
    ...
    for h in range(G):
        ...
        if not torch.allclose(output_lhs, output_rhs, atol=1e-5):
            ...
            print(
                f"      Max difference: {torch.max(torch.abs(output_lhs - output_rhs))}"
            )
            all_tests_passed = False
            break
